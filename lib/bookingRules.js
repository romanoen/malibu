...
function getPriceTier(duration) {
  ...
  return PRICE_TIERS.find(({ maxMinutes }) => totalMinutes <= maxMinutes);
}

function calculateBoardItemPrice(item, tier) {
  const basePricePerBoard = tier.pricePerBoard;
  const occupancySurchargePerBoard = item.peoplePerBoard === 2 ? TWO_PERSON_SURCHARGE_PER_BOARD : 0;
...
function calculatePrice(duration, boardItemsOrNumberOfBoards, boardTypeValue = 'allround', peoplePerBoardValue) {
  const boardItemsResult = Array.isArray(boardItemsOrNumberOfBoards)
    ...
    : normalizeBoardItems({
        ...
        peoplePerBoard: peoplePerBoardValue
      });
  ...
  const tier = getPriceTier(duration);
  const pricedBoardItems = boardItemsResult.boardItems.map(item => calculateBoardItemPrice(item, tier));
